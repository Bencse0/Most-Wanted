...
function startHunterGPS() {
  if (!navigator.geolocation) { document.getElementById('hunter-gps-status').innerText='VADÁSZ GPS: NEM TÁMOGATOTT'; return; }
  ...
  },()=>{document.getElementById('hunter-gps-status').innerText='VADÁSZ GPS: NEM ELÉRHETŐ';},{enableHighAccuracy:true,timeout:15000,maximumAge:1000});
  setInterval(postHunterLocation,1000);
}
function updateOwnHunterSpeed(){ const s=hunterPosition?.coords?.speed; document.getElementById('stat-hunter-speed').innerText=Number.isFinite(s)?`${toKmh(s).toFixed(1)} km/h`:'—'; }
async function postHunterLocation(){
  ...
  try { const res=await fetch('/api/hunter/location',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({latitude:c.latitude,longitude:c.longitude,accuracy:c.accuracy,speed:c.speed})}); if(res.status===401) location.reload(); }
  catch{}
  finally{hunterPostInFlight=false;}
}
...
async function fetchState(){
  ...
  try{
    const res=await fetch('/api/state',{cache:'no-store'}); if(res.status===401)return; if(!res.ok)return;
...
function updateStats(){const active=runnersData.filter(r=>!isLate(r)).length;const wanted=runnersData.find(r=>r.is_most_wanted);document.getElementById('stat-runners').innerText=runnersData.length;document.getElementById('stat-active').innerText=active;document.getElementById('stat-wanted').innerText=wanted?wanted.name:'—';updateOwnHunterSpeed();}
...
function renderRunners(){
  ...
  Object.keys(markers).forEach(id=>{if(!ids.has(Number(id))){map.removeLayer(markers[id]);delete markers[id];}});
  if(!runnersData.length){list.innerHTML='<div class="empty-state"><strong>Még nincs menekülő</strong><span>A csatlakozó játékosok itt jelennek meg.</span></div>';return;}
  list.innerHTML=runnersData.map(r=>{
    const late=isLate(r), penalty=isPenaltyActive(r), coords=getMapCoordinates(r); ensureMarker(r,coords);
